...
left half of the screen and keep Folio on the right. Capture each page with one
...
2. **Set the capture area.** Folio freezes the screen. Drag a box around the page
...
The title bar has two extra buttons: **pin** keeps Folio on top, and **snap**
moves it to the right half of the screen.

## Where your notes live

The library defaults to `Documents\Folio`, and you can change it in Settings:
...
- App settings are kept separately in `%APPDATA%\Folio\settings.json`.

## Things to know

- **Capture comes out black.** Some readers block screenshots. Folio warns you
...
- **Hotkey conflicts.** If another app already owns Ctrl+Alt+B, change it in
  Settings.
- **Keep Folio out of the area.** If the capture area overlaps Folio's own window,
  Folio warns you, because the window would end up in the screenshot.
...
Test hooks (environment variables): `FOLIO_USERDATA` and `FOLIO_LIBRARY` point
the app at throwaway folders. `FOLIO_SMOKE=<module.js>` runs a script that drives
